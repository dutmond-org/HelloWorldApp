...
pipeline {	
	...
	stages {
		...
		stage (Deploy) {
			steps {
					sshPublisher(publishers: [sshPublisherDesc(configName: 'cloud-54.188.156.202', transfers: [sshTransfer(excludes: '', execCommand: 'mv /home/user/opt/tomcat/apache-tomcat-8.5.33/webapps/HelloWorldApp.war /opt/tomcat/apache-tomcat-8.5.33/webapps/', execTimeout: 120000, flatten: false, makeEmptyDirs: false, noDefaultExcludes: false, patternSeparator: '[, ]+', remoteDirectory: '/opt/tomcat/apache-tomcat-8.5.33/webapps', remoteDirectorySDF: false, removePrefix: 'target', sourceFiles: 'target/*.war')], usePromotionTimestamp: false, useWorkspaceInPromotion: false, verbose: false)])
			    }
			}
	}
}
